...
const long long MAX_SIZE = 1006LL; vector<long long >isprime(MAX_SIZE , true); vector<long long >prime; vector<long long >SPF(MAX_SIZE); void manipulated_seive(int N) { isprime[0] = isprime[1] = false ; for (long long int i=2; i<N ; i++) { if (isprime[i]) { prime.push_back(i); SPF[i] = i; } for (long long int j=0; j < (int)prime.size() && i*prime[j] < N && prime[j] <= SPF[i]; j++) { isprime[i*prime[j]]=false; SPF[i*prime[j]] = prime[j] ; } } }
...
void solve()
{
    ...
    for(ll i=0; prime[i]*prime[i]<=n && i<sz;i++)
    {
        ...
        if(n%prime[i]==0)
        {
            ...
            c++;
            sod*= (pow(prime[i],c)-1)/(prime[i]-1);
        }


    }

    if(n>1) sod*=(pow(n,2LL)-1)/(n-1);
    printf("%lld\n",sod-sz);
...
int main()
{
    ...
    //freopen("output.cpp","r",stdin);
     //freopen("input.cpp","w",stdout);

        manipulated_seive(1003LL);
        sz=prime.size();
